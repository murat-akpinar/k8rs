...
| Add a **fixture** | [`scripts/broken.yaml`](../scripts/broken.yaml) → `cluster.sh verify` → `just fixtures`. Never write the JSON by hand. **Two guards, two questions**: `verify`'s predicate asks whether the *live* object reached the state its rule is about, and a cycling container answers yes on either face, so it stays loose; the `guard` in `just fixtures` asks whether the *committed bytes* can carry the tests written against them, so it names one face and `fetch_until` re-fetches until it lands ([D131](../NOTES.md#d131--a-fixture-is-asked-two-different-questions-and-one-disjunction-was-answering-both-2026-08-21)) | `tester` writes the manifest and the sanitizer; **the PM runs the capture** ([D92](../NOTES.md#d92--who-may-touch-a-cluster-split-by-the-artifact-and-not-by-the-agent-2026-08-15)) |
